...
export default function ScoreDetails() {
  ...
  const currentDayTotal   = currentDayEntries.reduce((sum, entry) => sum + (entry.points || 0), 0);
  // ───────────────────
  ...
  const dm = {
    ...
    input:     darkMode ? "bg-gray-700 border-gray-600 text-gray-100 placeholder-gray-400 focus:ring-blue-500" : "bg-white border-gray-300 text-gray-700 focus:ring-blue-300",
    tableHead: darkMode ? "bg-gray-700 text-gray-300"         : "bg-gray-50 text-gray-500",
    row:       darkMode ? "border-gray-700 hover:bg-gray-700" : "border-gray-100 hover:bg-blue-50",
    ...
    <div className={`min-h-screen border-2 border-dashed rounded-xl font-mono transition-colors duration-300 ${dm.page}`}>
      ...
      <div className="flex min-h-[calc(100vh-73px)] relative">
          ...
          {navItems.map((item, index) => (
            <button key={item}
              onClick={() => {
                ...
                navigate(url[index], {
                  state: { username, users, school, schoolName, programName, darkMode },
                });
              }}
        ...
        <div className={`flex-1 px-4 md:px-10 py-6 md:py-8 flex flex-col gap-5 md:gap-6 ${dm.main}`}>
              ...
              {undoStack.length > 0 && (
                <button onClick={handleRestore}
                  className={`text-xs font-bold px-3 py-1.5 rounded-lg active:scale-95 transition-all ${
                    darkMode ? "bg-yellow-900 text-yellow-200 hover:bg-yellow-800" : "bg-yellow-100 text-yellow-700 hover:bg-yellow-200"
                  }`}>
                  ↩ Restore Last
                </button>
              )}
          ...
          {/* School Info Banner */}
          <div className={`rounded-xl px-5 md:px-6 py-5 text-white shadow-lg transition-all duration-300 ${
            darkMode
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-4">
              {[
                ...
                <div key={item.label}
                  ...
                  <p className={`text-xs mb-1 ${darkMode ? "text-blue-300" : "text-blue-200"}`}>
                    {item.label}
                  </p>
                  <p className={`font-bold ${darkMode ? "text-gray-100" : "text-white"}`}>
                    {item.value}
                  </p>
                </div>
              ))}
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            {/* Day Navigator */}
            <div className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-bold shadow-md transition-all ${
              darkMode ? "bg-gray-700 text-gray-200 shadow-gray-900/30" : "bg-orange-400 text-white shadow-orange-400/30"
            }`}>
              <button onClick={() => setCurrentDay(Math.max(1, currentDay - 1))}
                className="hover:text-gray-100 transition-colors font-bold text-base">‹</button>
          ...
          <div className={`border rounded-xl shadow-sm overflow-hidden ${dm.card}`}>
            ...
            {/* Table Header */}
            <div className={`px-4 md:px-6 py-3 border-b flex items-center justify-between flex-wrap gap-2 ${
              darkMode ? "border-gray-700" : "border-gray-100"
            }`}>
              <h3 className={`text-sm font-bold ${dm.text}`}>
                📋 Event Log — Day {currentDay}&nbsp;
                <span className={`font-normal text-xs ${dm.subtext}`}>{currentDate}</span>
              </h3>
              <span className={`text-xs ${dm.subtext}`}>{currentDayEntries.length} entries</span>
            </div>
                ...
                <table className="w-full text-xs min-w-[560px]">
                  ...
                  <tbody>
                    ...
                    {currentDayEntries.map((eLog, index) => (
                      <tr key={index} className={`border-t transition-colors ${dm.row}`}>

                        <td className={`px-5 py-3 text-xs ${dm.subtext}`}>{eLog.time}</td>

                        <td className={`px-5 py-3 font-medium ${dm.cell}`}>{eLog.team}</td>

                        <td className={`px-5 py-3 ${dm.subtext}`}>{eLog.events}</td>
                  ...
                  <tfoot className={`border-t ${
                    ...
                    <tr>
                      <td colSpan={3} className={`px-5 py-2.5 text-xs font-bold uppercase tracking-wide ${dm.subtext}`}>
