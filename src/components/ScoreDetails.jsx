      ...
      <div className="flex min-h-[calc(100vh-73px)] relative">
        ...
        `}>
          <div className="flex items-center justify-between md:hidden mb-2">
            ...
            <button onClick={() => setSidebarOpen(false)} className="text-gray-400 hover:text-gray-600 font-bold text-lg">
              ×
            </button>
          </div>
          {navItems.map((item, index) => (
            <button
              ...
              onClick={() => {
                ...
                navigate(url[index], { state: { username: username, users: users, school: school, schoolName: schoolName, programName: programName, darkMode: darkMode, } });
              }}
        ...
        <div className={`flex-1 px-4 md:px-10 py-6 md:py-8 flex flex-col gap-5 md:gap-6 ${dm.main}`}>
          ...
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div>
              <h2 className={`text-base md:text-lg font-bold ${dm.text}`}>Score Details</h2>
              <p className={`text-xs mt-0.5 ${dm.subtext}`}>Live event log and scoreboard</p>
            </div>
            <div className="flex gap-2 items-center flex-wrap">
              {undoStack.length > 0 && (
                <button
                  onClick={handleRestore}
                  className={`text-xs font-bold px-3 py-1.5 rounded-lg active:scale-95 transition-all ${darkMode
              ...
              <span className={`text-xs font-bold px-3 py-1 rounded-full ${darkMode
          ...
          {/* School Info Banner - FIXED COLORS */}
          <div className={`rounded-xl px-6 py-5 text-white shadow-lg transition-all duration-300 ${darkMode
            ...
            <p className="text-xs font-bold uppercase tracking-widest text-blue-200 dark:text-blue-300 mb-3">School Details</p>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {[
                ...
                <div key={item.label} className={`rounded-lg px-4 py-3 backdrop-blur-sm transition-all ${darkMode
                  ? "bg-slate-600/60 shadow-lg shadow-slate-900/30"
                  : "bg-blue-500/80 shadow-md shadow-blue-500/40"
                  }`}>
                  <p className={`text-xs mb-1 transition-colors ${darkMode ? "text-blue-300" : "text-blue-200"
                    }`}>{item.label}</p>
                  <p className={`font-bold transition-colors ${darkMode ? "text-gray-100" : "text-white"
                    }`}>{item.value}</p>
                </div>
              ))}
            </div>
          ...
          <div className="flex items-center gap-4 flex-wrap">
            <button
              onClick={() => navigate("/scoreboard-mode1", { state: { username: username, users: users, school: school, schoolName: schoolName, programName: programName, darkMode: darkMode } })}
              className={`font-bold text-sm px-6 py-2.5 rounded-full active:scale-95 transition-all shadow-md hover:shadow-lg ${darkMode
            ...
            <div className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-bold shadow-md transition-all ${darkMode
              ? "bg-gray-700 text-gray-200 shadow-gray-900/30"
              : "bg-orange-400 text-white shadow-orange-400/30"
              }`}>
              <button
                onClick={() => setCurrentDay(Math.max(1, currentDay - 1))}
                className={`hover:text-gray-100 transition-colors ${darkMode ? "" : "hover:text-orange-100"}`}
              >
                ‹
              </button>
            ...
            <div className="ml-auto">
              <button
                onClick={() => navigate("/add-points", { state: { username: username, users: users, school: school, schoolName: schoolName, programName: programName, darkMode: darkMode } })}
                className={`text-sm font-bold px-5 py-2 rounded-lg active:scale-95 transition-all shadow-md hover:shadow-lg ${darkMode
          ...
          <div className={`border rounded-xl shadow-sm overflow-hidden ${dm.card}`}>
            <div className={`px-4 md:px-6 py-3 border-b flex items-center justify-between flex-wrap gap-2 ${darkMode ? "border-gray-700" : "border-gray-100"}`}>
              <h3 className={`text-sm font-bold ${dm.text}`}>
                ...
                <span className={`${dm.subtext} font-normal text-xs`}>{currentDate}</span>
              </h3>
              <span className={`text-xs ${dm.subtext}`}>{logs.length} entries</span>
            </div>

            {logs.length === 0 ? (
              <div className="px-6 py-12 text-center">
                <p className="text-3xl mb-3">📋</p>
                <p className={`text-sm font-bold ${dm.subtext}`}>No events logged yet.</p>
                <p className={`text-xs mt-1 ${dm.subtext}`}>Click "+ Log Event" to add the first entry.</p>
              </div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-xs min-w-[700px]">
                  ...
                  <>
                    <tbody>
                      {logs.map((log) =>
                        log[currentDay - 1].map((eLog, index) => (
                          <tr
                            key={index}
                            className={`border-t transition-colors ${dm.row}`}
                          >
                            <td className={`px-5 py-3 ${dm.subtext} text-xs`}>
                              {eLog.time}
                            </td>
                            ...
                            <td className="px-5 py-3">
                              <span
                                className={`px-2.5 py-0.5 rounded-full text-xs font-bold shadow-sm ${eLog.points >= 20
                                    ? darkMode
                    ...
                    >
                      <tr>
                        ...
                        <td colSpan={2} className="px-5 py-2.5">
                          <span
                            className={`text-xs font-bold px-3 py-1 rounded-full shadow-md ${darkMode
