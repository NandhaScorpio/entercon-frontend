...
const DEFAULT_TEAMS = [
  ...
  "Loyal Lions", "Fierce Falcons", "Sharp Sharks",
];
...
export default function AddSchools() {
  ...
  const handleEditSchool = (index) => {
    ...
    setDayCount(s.numberOfDays); setParticipants(s.participants);
        ...
        <div className={`flex-1 px-4 md:px-10 py-6 md:py-8 flex flex-col gap-5 md:gap-6 ${dm.main}`}>
          ...
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div>
              ...
              <p className={`text-xs mt-0.5 ${dm.subtext}`}>Register school details and assign teams in one place</p>
            </div>
              ...
              <div className={`border rounded-xl p-4 md:p-6 shadow-sm ${dm.card}`}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-5">
                  <div>
                    ...
                    <input type="text" placeholder="Enter School Name" value={schoolName}
                      onChange={(e) => setSchoolName(e.target.value)} className={inputClass} />
                  </div>
                  <div>
                    <label className={labelClass}>Program Name</label>
                    <input type="text" placeholder="Enter Program Name" value={programName}
                      ...
                      <tbody>
                        {schools.map((s, i) => (
                          <tr key={i} className={`border-t transition-colors ${dm.trow}`}>
                            <td className={`px-5 py-2.5 font-medium ${dm.tcell}`}>{s.schoolName}</td>
                            <td className={`px-5 py-2.5 ${dm.subtext}`}>{s.programName}</td>
                            <td className={`px-5 py-2.5 ${dm.subtext}`}>{s.numberOfDays}</td>
                            <td className="px-5 py-2.5">
                              <span className="bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full font-bold">{s.participants}</span>
                            </td>
                            <td className={`px-5 py-2.5 ${dm.subtext}`}>{s.startDate} → {s.endDate}</td>
                            <td className="px-5 py-2.5">
          ...
          {activeStep === 2 && (
            <div className="flex flex-col gap-5">
              ...
              <div className={`border rounded-xl shadow-sm overflow-hidden ${dm.card}`}>
                <div className={`flex items-center justify-between px-5 py-3 border-b flex-wrap gap-2 ${darkMode ? "border-gray-700" : "border-gray-100"}`}>
                  <h3 className={`text-sm font-bold ${dm.text}`}>🏷️ Team Names</h3>
                ...
                <div className="p-3 md:p-4 overflow-x-auto">
                  ...
                  <p className={`text-xs mt-2 text-center ${dm.subtext}`}>Click a team to select / deselect it</p>
                </div>
              </div>
              ...
              <div className={`border rounded-xl shadow-sm overflow-hidden ${dm.card}`}>
                ...
                {selectedTeams.length === 0 ? (
                  <div className={`px-6 py-8 text-center text-sm ${dm.subtext}`}>
